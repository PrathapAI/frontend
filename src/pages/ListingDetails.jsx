    ...
    <div className="page-bg-blue-accent">
      <div className="home-main-container">
        <main className="home-listings-content" style={{ maxWidth: 900, margin: '0 auto', padding: 32 }}>
          ...
          {/* Chat button for logged-in users who are not the owner */}
          {user && listing.User && user.id !== listing.User.UserID && (
            <button style={{margin:'16px 0',padding:'10px 24px',background:'#111',color:'#111',border:'none',borderRadius:8,fontWeight:700,cursor:'pointer'}} onClick={()=>setShowChat(true)}>
              Message Seller
            </button>
          )}
          {/* Owner view: see all chats for this listing (not implemented here, but placeholder) */}
          {user && listing.User && user.id === listing.User.UserID && (
            <OwnerChats listingId={listing.ListingID || listing.id} user={user} />
          )}
          {/* Chat modal */}
          {showChat && user && (
            <Chat listingId={listing.ListingID || listing.id} receiverId={listing.User.UserID} user={user} onClose={()=>setShowChat(false)} />
          )}
        </main>
      </div>
    </div>
